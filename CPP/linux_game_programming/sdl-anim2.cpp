...
// Previous using a double bufferred window/surface and rgb correct surfaces.
// 7. sdl-anim2.cpp
...
static SDL_Surface *temp, *screen, *actor;
...
int main(int argc, char* argv[]) {
    ...
    temp = IMG_Load("resource/img/640bg.bmp");
    SDL_Surface *background = SDL_DisplayFormat(temp);
    temp = IMG_Load("resource/img/actor.png");
    actor = SDL_DisplayFormat(temp);
    if (!background || !actor) {
        cout << "Image Error: " << SDL_GetError();
        return 1;
    }
    
    // get rid of it to avoid memory leak
    ...
    dest.x = 0;
    dest.y = 0;    
    for (int frames = 0; frames < 444; frames++) {
